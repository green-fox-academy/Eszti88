...
function getNameAndBalance(accNum: number) {
  let nameAndBalanceList: any[] = [];
  Object.keys(accounts).forEach(index => {
    if (accNum === accounts[index].accountNumber) {
      nameAndBalanceList.push(accounts[index].clientName, accounts[index].balance);
    }
  });
